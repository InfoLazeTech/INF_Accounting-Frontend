...
const CustomInput = ({
  ...
  options = [],
  rules = [],
  otpLength = 6, // allow configurable OTP length
  ...restProps
}) => {
  ...
  switch (type) {
    case 'password':
      inputComponent = <Input.Password placeholder={placeholder} {...restProps} />;
      break;
    case 'textarea':
      inputComponent = <TextArea placeholder={placeholder} {...restProps} />;
      break;
    case 'number':
      inputComponent = <InputNumber placeholder={placeholder} style={{ width: '100%' }} {...restProps} />;
      break;
    case 'select':
      inputComponent = <Select placeholder={placeholder} options={options} {...restProps} />;
      break;
    case 'date':
  ...
  const valuePropName =
    type === 'switch' ? 'checked' :
    type === 'file' ? 'fileList' :
    type === 'checkbox' && options.length === 0 ? 'checked' : 'value';
