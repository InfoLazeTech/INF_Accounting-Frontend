...
function Transaction() {
  ...
  const columns = [
    ...
    //   key: "typeDisplay",
    //   render: (_, record) => {
    //     const parentType = record?.parentId?.parenttype;
    //     const customerVendorName = record?.customerVendorId?.companyName;
    //     const childName = record?.childrenId?.accountname;

    //     return parentType ? (
    //       <Tag color="blue">{parentType}</Tag>
    //     ) : customerVendorName ? (
    //       <Tag color="green">{customerVendorName}</Tag>
    //     ) : childName ? (
    //       <Tag color="purple">{childName}</Tag>
    //     ) : (
    //       "-"
    //     );
    //   },
  ...
  render: (_, record) => {
    ...
    if (parentType && childName) {
      return (
        <Tag color="blue">
          {parentType} → <strong>{childName}</strong>
        </Tag>
      );
    }

    if (parentType) {
      return <Tag color="blue">{parentType}</Tag>;
    }

    if (customerVendorName) {
      return <Tag color="green">{customerVendorName}</Tag>;
    }
    return "-";
  },
},
          ...
          <Col span={8}>
            <Select
              ...
              onChange={(value) => {
                ...
                value
                //   ? navigate(`/banking/transaction/${value}`)
                //   : navigate(`/banking`);
              }}
        ...
        <Row gutter={16} style={{ marginBottom: 16 }}>
          <Col span={6}>
            <Card>
              ...
              <div className="text-2xl font-bold">{bankData?.bankName}</div>
            </Card>
          </Col>
          <Col span={6}>
            <Card>
